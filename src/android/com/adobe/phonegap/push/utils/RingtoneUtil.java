...
public final class RingtoneUtil {
    ...
    private static final String TAG = RingtoneUtil.class.toString();
    ...
    public static @Nullable Ringtone getRingtone(@NonNull Context context, @NonNull Uri uri) {
        ...
        try {
            ...
            Logger.Error(TAG, "getActualDefaultRingtoneUri", "Unable to get default ringtone due to permission: " + e);
    ...
    public static @Nullable Uri getActualDefaultRingtoneUri(@NonNull Context context) {
        Logger.Debug(TAG, "getActualDefaultRingtoneUri", "Attempting to get default ringtone directly via normal way");
        try {
            return RingtoneManager.getActualDefaultRingtoneUri(context, RingtoneManager.TYPE_RINGTONE);
        } catch (SecurityException e) {
            Logger.Error(TAG, "getActualDefaultRingtoneUri", "Failed to get ringtone with first fallback approach: " + e);
        }

        Logger.Debug(TAG, "getActualDefaultRingtoneUri", "Attempting to get default ringtone directly via reflection");
    ...
    private static @Nullable String getStringForUser(@NonNull ContentResolver resolver, int userHandle) {
        try {
            ...
            Logger.Error(TAG, "getActualDefaultRingtoneUri", "Unable to getStringForUser via reflection: " + e);
        }
        return null;
    }
    ...
    private static int getUserId(@NonNull Context context) {
        try {
            ...
            if (userId instanceof Integer) {
                ...
                Logger.Error(TAG, "getActualDefaultRingtoneUri", "getUserId did not return an integer");
            }
        } catch (IllegalAccessException | InvocationTargetException | NoSuchMethodException e) {
            Logger.Error(TAG, "getActualDefaultRingtoneUri", "Unable to getUserId via reflection: " + e);
        }
        return 0;
    }
