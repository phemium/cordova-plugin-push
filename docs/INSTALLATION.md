...
```xml
<plugin name="@phemium-costaisa/cordova-plugin-push" spec="3.0.0" />
```
...
```json
{
  "cordova": {
    "plugins": {
      "@phemium-costaisa/cordova-plugin-push": {
...
When some other packages include `cordova-google-play-services` as a dependency, such as is the case with the `cordova-admob` and `cordova-plugin-analytics` plugins, it is impossible to also add the `@phemium-costaisa/cordova-plugin-push`, for the following error will rise during the build process:
...
```log
Installing "@phemium-costaisa/cordova-plugin-push" for ios
Failed to install '@phemium-costaisa/cordova-plugin-push':Error: pod: Command failed with exit code 1
